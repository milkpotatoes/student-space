...
function randomFunName() {
    ...
    let name = ""
    str = str.split("")
    name = Math.round(Math.random() * 26)
    for (let i = 0; i < 3; i++) {
...
document.addEventListener("click", (e) => {
    if (e.target == document.querySelector(".logout")) {
        alertDialg("退出登录", "", () => {
            io.emit("recall_device", getCookie("DevicesUUID"));
        });
    }
    if (e.target in document.querySelectorAll(".claim-exam")) {
    ...
    if (e.target in document.querySelectorAll(".dialog>.buttons>.circle")) {
